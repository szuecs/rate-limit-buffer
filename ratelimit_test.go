...
func TestClientRateLimiterDeleteOld(t *testing.T) {
	...
	rl.Allow(context.Background(), "foo")
	rl.Allow(context.Background(), "bar")
	rl.DeleteOld()
...
func TestClientRateLimiterOldest(t *testing.T) {
	...
	rl.Allow(context.Background(), "foo") // [t0, 0]
	if !rl.Oldest("foo").Equal(zero) {
		t.Errorf("1foo should return zero")
	}

	t0 := rl.Current("foo")
	rl.Allow(context.Background(), "foo") // [t0, t1]
...
func TestClientRateLimiterAllow(t *testing.T) {
	...
	if !rl.Allow(context.Background(), "foo") {
	...
	if !rl.Allow(context.Background(), "bar") {
...
func TestClientRateLimiterAllowConcurrent(t *testing.T) {
	...
	wg.Add(3)
	f := func(s string) {
		if !rl.Allow(context.Background(), s) {
			t.Errorf("%s should not be rate limitted", s)
		}
		if !rl.Allow(context.Background(), s) {
			t.Errorf("%s should not be rate limitted", s)
...
func BenchmarkClientRateLimiterAllow(b *testing.B) {
	...
	for n := 0; n < b.N; n++ {
		if !rl.Allow(context.Background(), "foo") && !rl.Allow(context.Background(), fmt.Sprintf("foo%d", n)) {
...
func BenchmarkClientRateLimiterAllowBaseData1(b *testing.B) {
	...
	m := 10
	for i := 0; i < m; i++ {
		rl.Allow(context.Background(), fmt.Sprintf("foo%d", i%m))
	}

	for n := 0; n < b.N; n++ {
		rl.Allow(context.Background(), fmt.Sprintf("foo%d", n%m))
	}
	rl.Close()
}
...
func BenchmarkClientRateLimiterAllowConcurrent1(b *testing.B) {
	...
	for i := 0; i < 1; i++ {
		...
		go func(j int) {
			for n := 0; n < b.N; n++ {
				rl.Allow(context.Background(), fmt.Sprintf("foo%d", (j+n)%m))
			}
			wg.Done()
		}(i)
	}
